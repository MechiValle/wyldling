...
export function Search() {
  ...
  useEffect(() => {
    ...
    async function runSearch() {
      ...
      const [itemsResult, charactersResult, shopsResult] = await Promise.all([
        ...
        supabase
          .from('shops')
          .select('id, name, characters(image_path)')
          .ilike('name', `%${trimmed}%`),
      ])
      ...
      const characterResults: SearchResult[] = (charactersResult.data ?? []).map((c) => ({
        id: c.id,
        name: c.name,
      ...
      const shopRows = (shopsResult.data ?? []) as unknown as {
        id: number
        name: string
        characters: { image_path: string | null } | null
      }[]
  ...
  function goToResult(result: SearchResult) {
    const routeByType: Record<ResultType, string> = {
      fish: '/fish',
      food: '/food',
      animal: '/animals',
    ...
    navigate(`${routeByType[result.type]}?open=${result.id}`)
  }
    ...
    <div>
      ...
      <div className="flex flex-col gap-2 max-w-md">
        {results.map((result) => (
          ...
          >
            <img src={result.image} alt={result.name} className="w-10 h-10 object-contain rounded-md flex-shrink-0" />
            <span className="font-body flex-1">{result.name}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full border capitalize ${typeTagStyles[result.type]}`}>
              {result.type}
            </span>
          </button>
        ))}
