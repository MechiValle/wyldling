...
export function Food() {
  ...
  useEffect(() => {
    ...
    async function fetchFood() {
      ...
      const EXCLUDED_CATEGORIES = ['Store-Bought', 'Fermented/Distilled'];

      const foodWithCategory: FoodWithCategory[] = foodItems
        .map((item) => ({
          ...item,
          categoryName: categoryByItem.get(item.id) ?? null,
        }))
        .filter(
          (item) => !EXCLUDED_CATEGORIES.includes(item.categoryName ?? ''),
        );

      await preloadImages(
        foodWithCategory.map((item) => getImageUrl(item.image_path)),
      );

      setFood(foodWithCategory);
      setCategoryOptions([
        'All',
        ...(categoriesData as { id: number; name: string }[])
          .map((c) => c.name)
