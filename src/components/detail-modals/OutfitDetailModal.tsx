...
export function OutfitDetailModal({ outfit, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const { data, error } = await supabase
        ...
        .select(
          'material_item_id, quantity, items!material_item_id(name, image_path)',
        )
        .eq('outfit_item_id', outfit.id);
          ...
          <div className='flex flex-wrap justify-center gap-3'>
            {materials.map((m) => (
              ...
              >
                ...
                />
                <span className='text-xs text-center w-full wrap-break-words leading-tight'>
                  {m.items?.name}
                </span>
                <span className='text-xs font-semibold text-marigold-harvest'>
                  ×{m.quantity}
                </span>
              </div>
            ))}
